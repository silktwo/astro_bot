...
- [ ] **Step 2:** Створити `~/astro-bot/.env` на сервері з реальними значеннями:
  ...
  `ADMIN_TELEGRAM_ID=281735938`, `PUSH_HOUR=18`, `PUSH_TZ=Europe/Warsaw`.
  `GF_TELEGRAM_ID` НЕ потрібен — чат привʼязується автоматично на `/start`.
  Файл — права 600, у git не потрапляє.

- [ ] **Step 3:** Підняти стек через **Arcane** (compose) або `docker compose up -d --build`.
  Перевірити `restart: unless-stopped`. Карта Анни сідиться автоматично з
  `data/anna_natal.json` при першому `/start` (`SEED_PATH=/app/data/anna_natal.json`).

- [ ] **Step 4: Жива перевірка** — Анна тисне `/start` (бот сам запамʼятовує її чат і
  привʼязує карту), потім `/today`; очікувати прогноз українською: спільні транзитні
  аспекти + подвійне трактування знаків (тропіка ♋ / сидерика ♊ тощо).
...
- [ ] **Step 6: Перевірка пушу** — тимчасово виставити `PUSH_HOUR` на найближчу
  хвилину, переконатися що прогноз приходить у її чат автоматично (без ID);
  повернути `PUSH_HOUR=18`.
